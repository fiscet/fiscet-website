...
The part I find genuinely smart is something else: because it's software, the camera's "views" can be updated even after the product is already in the field. A hardware ISP, once soldered, is what it is forever. Maybe with this approach cars, drones, and security cameras will see better in the dark without fitting bigger, pricier sensors, because the gain comes from the model, not the glass.
